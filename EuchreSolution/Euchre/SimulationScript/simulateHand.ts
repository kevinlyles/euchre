function startSimulation(): void {
	...
	let trumpCandidateCard = getCard("trumpCandidate", "Trump candidate card");
	...
	startWorker(hand, trumpCandidateCard, dealer, orderItUp, discard, suitToCall, goAlone);
}
...
function updateActions(): void {
	...
	let trumpCandidateCard = getCard("trumpCandidate");
	updateDiscardCard(6, trumpCandidateCard);

	let orderItUpCheckbox = document.getElementById("orderItUp") as HTMLInputElement;
	let hasSuit = false;
	if (trumpCandidateCard) {
		for (let card of hand) {
			if (card && card.suit === trumpCandidateCard.suit) {
				hasSuit = true;
				break;
			}
		}
...
function buildDeck(hand: Card[], trumpCandidateCard: Card): Card[] {
	let idsToSkip = { [trumpCandidateCard.id]: true };
	for (let card of hand) {
...
/*function startWorker(hand: Card[], trumpCandidateCard: Card, dealer: Player.South, orderItUp: true, discard: Card, suitToCall: null, goAlone: boolean): void
function startWorker(hand: Card[], trumpCandidateCard: Card, dealer: Player.North | Player.East | Player.West, orderItUp: true, discard: null, suitToCall: null, goAlone: boolean): void
function startWorker(hand: Card[], trumpCandidateCard: Card, dealer: Player, orderItUp: false, discard: null, suitToCall: Suit, goAlone: boolean): void
function startWorker(hand: Card[], trumpCandidateCard: Card, dealer: Player, orderItUp: false, discard: null, suitToCall: null, goAlone: false): void*/
function startWorker(hand: Card[], trumpCandidateCard: Card, dealer: Player,
	orderItUp: boolean, discard: Card | null, suitToCall: Suit | null,
	goAlone: boolean): void {
	let deck = buildDeck(hand, trumpCandidateCard);
