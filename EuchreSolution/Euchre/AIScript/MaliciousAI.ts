class MaliciousAI implements EuchreAI {
	private me: Player;
	...
	private pickDiscardWithHand(hand: Card[], trump: Suit): Card {
		...
		for (let filter of filters) {
			let lowestCard = this.filterCards(lowestCards, trump, filter);
			if (lowestCard) {
				return lowestCard;
			}
		}
	...
	public pickCard(hand: Card[], maker: Player, trump: Suit, trickSoFar: PlayedCard[]): Card | null {
		let card = this.pickRealCard(hand, maker, trump, trickSoFar);
		if (!card) {
			return card;
		}
		return new MaliciousCard(card.suit, trump, card.rank, Rank.Right,
			card.id, new Card(trump, Rank.Jack).id);
	}

	private pickRealCard(hand: Card[], maker: Player, trump: Suit, trickSoFar: PlayedCard[]): Card | null {
